...
public class Game extends Activity {
	...
	private boolean fixedTiles[] = new boolean[81];
	...
	protected void showKeypadOrError(int x, int y) {
		if (!isFixedTile(x, y)) {
			...
			Log.d(TAG, "cannot change fixed tile at " + x + ", " + y);
		}
	}
	protected boolean isUserStuck(int x, int y) {
		int tiles[] = getUsedTiles(x, y);
		 return tiles.length == 9;
	}
	protected boolean isFixedTile(int x, int y) {
		return fixedTiles[y * 9 + x];
	}
	...
	private void calculateFixedTiles() {
		for (int i = 0; i < 81; i++) {
			if (puzzle[i] != 0)
				fixedTiles[i] = true;
		}
